...
_EPOCH_DELTA_1970_2000 = const(
    946684800
)  # Seconds between 1970 and 2000 = _EPOCH_DELTA_1900_2000 - _EPOCH_DELTA_1900_1970
...
class Ntp:
    ...
    _drift_last_compensate: int = (
        0  # Last RTC drift compensation timestamp. Uses device's epoch
    )
    _drift_last_calculate: int = (
        0  # Last RTC drift calculation timestamp. Uses device's epoch
    )
    ...
    _dst_cache_switch_hours_timestamp = (
        None  # Cache the year, the last switch time calculation was made
    )
    ...
    __epoch_delta_lut = (
        (0, -_EPOCH_DELTA_1900_1970, -_EPOCH_DELTA_1900_2000),
    ...
    def set_datetime_callback(cls, callback, precision=SUBSECOND_PRECISION_US):
        ...
        if not callable(callback):
            raise ValueError(
                "Invalid parameter: callback={} must be a callable object".format(
                    callback
                )
            )

        if precision not in (
            cls.SUBSECOND_PRECISION_SEC,
            cls.SUBSECOND_PRECISION_MS,
            cls.SUBSECOND_PRECISION_US,
        ):
            raise ValueError(
                "Invalid parameter: precision={} must be one of SUBSECOND_PRECISION_SEC, SUBSECOND_PRECISION_MS, SUBSECOND_PRECISION_US".format(
                    precision
                )
            )
        ...
        def precision_adjusted_callback(*args):
            ...
            raise ValueError(
                f"Invalid parameters: {args}. In setter mode expects 8-tuple"
            )
    ...
    def set_logger_callback(cls, callback=print):
        ...
        if callback is not None and not callable(callback):
            raise ValueError(
                "Invalid parameter: callback={} must be a callable object or None to disable logging".format(
                    callback
                )
            )
    ...
    def set_epoch(cls, epoch: int = None):
        ...
        if epoch is None:
            ...
            raise ValueError(
                "Invalid parameter: epoch={} must be a one of Ntp.EPOCH_1900, Ntp.EPOCH_1970, Ntp.EPOCH_2000 or None".format(
                    epoch
                )
            )
    ...
    def set_dst(cls, start: tuple = None, end: tuple = None, bias: int = 0):
        ...
        if start is None or end is None or bias == 0:
            ...
            raise ValueError(
                "Invalid parameter: start={} must be a 4-tuple(month, week, weekday, hour)".format(
                    start
                )
            )
        elif not isinstance(end, tuple) or len(end) != 4:
            raise ValueError(
                "Invalid parameter: end={} must be a 4-tuple(month, week, weekday, hour)".format(
                    end
                )
            )
        else:
    ...
    def set_dst_start(cls, month: int, week: int, weekday: int, hour: int):
        ...
        if not isinstance(month, int) or not cls.MONTH_JAN <= month <= cls.MONTH_DEC:
            raise ValueError(
                "Invalid parameter: month={} must be a integer in (Ntp.MONTH_JAN ... Ntp.MONTH_DEC)".format(
                    month
                )
            )
        elif not isinstance(week, int) or not cls.WEEK_FIRST <= week <= cls.WEEK_LAST:
            raise ValueError(
                "Invalid parameter: week={} must be a integer in (Ntp.WEEK_FIRST ... Ntp.WEEK_LAST)".format(
                    week
                )
            )
        elif (
            not isinstance(weekday, int)
            or not cls.WEEKDAY_MON <= weekday <= cls.WEEKDAY_SUN
        ):
            raise ValueError(
                "Invalid parameter: weekday={} must be a integer in (Ntp.WEEKDAY_MON ... Ntp.WEEKDAY_SUN)".format(
                    weekday
                )
            )
        elif not isinstance(hour, int) or not 0 <= hour <= 23:
            raise ValueError(
                "Invalid parameter: hour={} must be a integer between 0 and 23".format(
                    hour
                )
            )
    ...
    def set_dst_bias(cls, bias: int):
        ...
        if not isinstance(bias, int) or bias not in (0, 30, 60, 90, 120):
            raise ValueError(
                "Invalid parameter: bias={} represents minutes offset and must be either 0, 30, 60, 90 or 120".format(
                    bias
                )
            )
    ...
    def dst(cls, dt=None):
        ...
        if dt is None:
            ...
            raise ValueError(
                "Invalid parameter: dt={} must be a 8-tuple(year, month, day, weekday, hour, minute, second, subsecond)".format(
                    dt
                )
            )
        ...
        if (
            dt[0] != cls._dst_cache_switch_hours_timestamp
            or cls._dst_cache_switch_hours_start is None
            or cls._dst_cache_switch_hours_end is None
        ):
            cls._dst_cache_switch_hours_timestamp = dt[0]
            cls._dst_cache_switch_hours_start = (
                cls.weekday_in_month(
                    dt[0], cls._dst_start[0], cls._dst_start[1], cls._dst_start[2]
                )
                * 24
                + cls._dst_start[3]
            )
            cls._dst_cache_switch_hours_end = (
                cls.weekday_in_month(
                    dt[0], cls._dst_end[0], cls._dst_end[1], cls._dst_end[2]
                )
                * 24
                + cls._dst_end[3]
            )
        ...
        if (
            cls._dst_start[0] < dt[1] < cls._dst_end[0]
            or (
                dt[1] == cls._dst_start[0]
                and (dt[2] * 24 + dt[4]) >= cls._dst_cache_switch_hours_start
            )
            or (
                dt[1] == cls._dst_end[0]
                and (dt[2] * 24 + dt[4]) < cls._dst_cache_switch_hours_end
            )
        ):
    ...
    def set_ntp_timeout(cls, timeout_s: int = 1):
        ...
        if not isinstance(timeout_s, int):
            raise ValueError(
                "Invalid parameter: timeout_s={} must be int".format(timeout_s)
            )
    ...
    def set_timezone(cls, hour: int, minute: int = 0):
        ...
        if not isinstance(hour, int) or not isinstance(minute, int):
            raise ValueError(
                "Invalid parameter: hour={}, minute={} must be integers".format(
                    hour, minute
                )
            )
    ...
    def time_us(cls, epoch: int = None, utc: bool = False):
        ...
        return (
            time.mktime((dt[0], dt[1], dt[2], dt[4], dt[5], dt[6], 0, 0, 0))
            + epoch_delta
            + timezone_and_dst
        ) * 1000_000 + dt[7]
    ...
    def ntp_time(cls, epoch: int = None):
        ...
        for host in cls._hosts:
            ...
            if (cls.__ntp_msg[0] & 0b00000111) != 4:
                cls._log(
                    '(NTP) Invalid packet due to bad "mode" field value: Host({})'.format(
                        host
                    )
                )
                continue
            ...
            if ((cls.__ntp_msg[0] >> 6) & 0b00000011) > 2:
                cls._log(
                    '(NTP) Invalid packet due to bad "leap" field value: Host({})'.format(
                        host
                    )
                )
                continue
            ...
            if not (1 <= (cls.__ntp_msg[1]) <= 15):
                cls._log(
                    '(NTP) Invalid packet due to bad "stratum" field value: Host({})'.format(
                        host
                    )
                )
                continue
            ...
            srv_receive_ts_sec, srv_receive_ts_frac = struct.unpack(
                "!II", cls.__ntp_msg[32:40]
            )  # T3
            ...
            srv_transmit_ts_sec, srv_transmit_ts_frac = struct.unpack(
                "!II", cls.__ntp_msg[40:48]
            )  # T4
            ...
            srv_receive_ts_us = srv_receive_ts_sec * 1_000_000 + (
                srv_receive_ts_frac * 1_000_000 >> 32
            )
            # Convert T4 to microseconds
            srv_transmit_ts_us = srv_transmit_ts_sec * 1_000_000 + (
                srv_transmit_ts_frac * 1_000_000 >> 32
            )
            # Calculate network delay in microseconds
            network_delay_us = (receive_ts_us - transmin_ts_us) - (
                srv_transmit_ts_us - srv_receive_ts_us
            )
            ...
            adjusted_server_time_us += (
                cls.epoch_delta(from_epoch=cls.EPOCH_1900, to_epoch=epoch) * 1_000_000
            )
    ...
    def rtc_sync(cls, new_time=None):
        ...
        if new_time is None:
            ...
            raise ValueError(
                "Invalid parameter: new_time={} must be a either None or 2-tuple(time, timestamp)".format(
                    new_time
                )
            )
        ...
        cls._datetime(
            (lt[0], lt[1], lt[2], lt[6], lt[3], lt[4], lt[5], ntp_us % 1000_000)
        )
        # Store the precision-adjusted value to match what RTC actually stores
        cls._rtc_last_sync = (
            ntp_us // cls._datetime_callback_precision
        ) * cls._datetime_callback_precision
    ...
    def rtc_last_sync(cls, epoch: int = None, utc: bool = False):
        ...
        return (
            0
            if cls._rtc_last_sync == 0
            else cls._rtc_last_sync + (epoch_delta + timezone_and_dst) * 1000_000
        )
    ...
    def drift_last_compensate(cls, epoch: int = None, utc: bool = False):
        ...
        return (
            0
            if cls._drift_last_compensate == 0
            else cls._drift_last_compensate
            + (epoch_delta + timezone_and_dst) * 1000_000
        )
    ...
    def drift_last_calculate(cls, epoch: int = None, utc: bool = False):
        ...
        return (
            0
            if cls._drift_last_calculate == 0
            else cls._drift_last_calculate + (epoch_delta + timezone_and_dst) * 1000_000
        )
    ...
    def set_drift_ppm(cls, ppm: float):
        ...
        if not isinstance(ppm, (float, int)):
            raise ValueError(
                "Invalid parameter: ppm={} must be float or int".format(ppm)
            )
    ...
    def drift_us(cls, ppm_drift: float = None):
        ...
        if not isinstance(ppm_drift, (float, int)):
            raise ValueError(
                "Invalid parameter: ppm_drift={} must be float or int".format(ppm_drift)
            )

        delta_time_rtc = cls.time_us(epoch=cls.device_epoch(), utc=True) - max(
            cls._rtc_last_sync, cls._drift_last_compensate
        )
    ...
    def drift_compensate(cls, compensate_us: int):
        ...
        if not isinstance(compensate_us, int):
            raise ValueError(
                "Invalid parameter: compensate_us={} must be int".format(compensate_us)
            )
        ...
        cls._datetime(
            (lt[0], lt[1], lt[2], lt[6], lt[3], lt[4], lt[5], rtc_us % 1000_000)
        )
    ...
    def weekday(cls, year: int, month: int, day: int):
        ...
        if not isinstance(year, int) or not 1 <= year:
            raise ValueError(
                "Invalid parameter: year={} must be int and greater than 1".format(year)
            )
        elif not isinstance(month, int) or not cls.MONTH_JAN <= month <= cls.MONTH_DEC:
            raise ValueError(
                "Invalid parameter: month={} must be int in range 1-12".format(month)
            )

        days = cls.days_in_month(year, month)
        if day > days:
            raise ValueError(
                "Invalid parameter: day={} is greater than the days in month({})".format(
                    day, days
                )
            )
        ...
        c = year // 100
        w = (
            int(day + int((13 * (month + 1)) / 5) + y + int(y / 4) + int(c / 4) + 5 * c)
            % 7
        )
    ...
    @classmethod
    def weekday_in_month(
        cls, year: int, month: int, ordinal_weekday: int, weekday: int
    ):
        ...
        if not isinstance(year, int) or not 1 <= year:
            ...
            not isinstance(ordinal_weekday, int)
            or not cls.WEEK_FIRST <= ordinal_weekday <= cls.WEEK_LAST
        ):
            raise ValueError(
                "Invalid parameter: ordinal_weekday={} must be int in range 1-6".format(
                    ordinal_weekday
                )
            )
        elif (
            ...
            raise ValueError(
                "Invalid parameter: weekday={} must be int in range 0-6".format(weekday)
            )

        first_weekday = cls.weekday(year, month, 1)  # weekday of first day of month
        first_day = (
            1 + (weekday - first_weekday) % 7
        )  # monthday of first requested weekday
        weekdays = [i for i in range(first_day, cls.days_in_month(year, month) + 1, 7)]
        return (
            weekdays[-1]
            if ordinal_weekday > len(weekdays)
            else weekdays[ordinal_weekday - 1]
        )
    ...
    def day_from_week_and_weekday(cls, year, month, week, weekday):
        ...
        if not isinstance(year, int) or not 1 <= year:
            ...
            raise ValueError(
                "Invalid parameter: week={} must be int in range 1-6".format(week)
            )
        elif (
            not isinstance(weekday, int)
            or not cls.WEEKDAY_MON <= weekday <= cls.WEEKDAY_SUN
    ...
    def epoch_delta(cls, from_epoch: int, to_epoch: int):
        ...
        elif not isinstance(from_epoch, int) or not (
            cls.EPOCH_1900 <= from_epoch <= cls.EPOCH_2000
        ):
            raise ValueError(
                "Invalid parameter: from_epoch={} must be a one of Ntp.EPOCH_1900, Ntp.EPOCH_1970, Ntp.EPOCH_2000, None".format(
                    from_epoch
                )
            )
        ...
        if to_epoch is None:
            to_epoch = cls._epoch
        elif not isinstance(to_epoch, int) or not (
            cls.EPOCH_1900 <= to_epoch <= cls.EPOCH_2000
        ):
            raise ValueError(
                "Invalid parameter: to_epoch={} must be a one of Ntp.EPOCH_1900, Ntp.EPOCH_1970, Ntp.EPOCH_2000, None".format(
                    to_epoch
                )
            )
    ...
    def _validate_hostname(hostname: str):
        ...
        if not isinstance(hostname, str):
            raise ValueError(
                "Invalid parameter: hostname={} must be a string".format(hostname)
            )
