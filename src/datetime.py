...
def _dbm(y, m):
    ...
    return (0, 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334)[m] + (
        m > 2 and _leap(y)
    )
...
timedelta.max = timedelta(
    days=999_999_999, hours=23, minutes=59, seconds=59, microseconds=999_999
)
...
def _iso2t(s):
    ...
    if tz_sign:
        td = timedelta(
            hours=tz_hour, minutes=tz_minute, seconds=tz_sec, microseconds=tz_usec
        )
        if tz_sign == "-":
            td = -td
        tz = timezone(td)
...
def _t2iso(td, timespec, dt, tz):
    s = td._fmt(
        ("auto", "hours", "minutes", "seconds", "milliseconds", "microseconds").index(
            timespec
        )
    )
    if tz is not None:
...
class time:
    def __init__(
        self, hour=0, minute=0, second=0, microsecond=0, tzinfo=None, *, fold=0
    ):
